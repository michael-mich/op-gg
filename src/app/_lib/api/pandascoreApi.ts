...
export const getLecSpringSeason = async (): Promise<Array<TLecSpringSeason> | void | undefined> => {
...
export const getMatchResultLionsVsFnatic = async (): Promise<TEsportMatch | void | undefined> => {
  const url = `https://api.pandascore.co/matches/636351?token=${pandascoreApiKey}`;
  return await fetchApi(url, { cache: 'force-cache' });
}

export const getMatchResultFnaticVsBds = async (): Promise<TEsportMatch | void | undefined> => {
