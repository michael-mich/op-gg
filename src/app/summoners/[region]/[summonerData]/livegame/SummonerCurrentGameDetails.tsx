...
type Props = {
  data: TUpdatedLiveGameParticipants;
}

const SummonerCurrentGameDetails = ({ data }: Props) => {
  return (
    <div className='flex items-center'>
      ...
      <div className='flex flex-col gap-[0.1rem] ml-3'>
        <Link href='' className='flex items-baseline gap-0.5 text-xs hover:underline'>
          <span className='font-bold text-[#202d37] dark:text-white'>{data.summonerNameAndTagLine?.name}</span>
          <span className='text-lightMode-secondLighterGray dark:text-darkMode-lighterGray'>#{data.summonerNameAndTagLine?.tagLine}</span>
        </Link>
